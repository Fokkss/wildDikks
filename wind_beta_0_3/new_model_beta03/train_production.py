...
def make_features_chrono(df: pd.DataFrame, target_col: str | None = None) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    fe = make_features(sorted_df.drop(columns=["__orig_order_beta03"], errors="ignore"), target_col=target_col)
...
def build_xgb(cfg: CandidateConfig, seed: int):
    try:
        ...
        raise RuntimeError("xgboost is required for beta 0.3 production training") from e
    return XGBRegressor(
...
def train_and_predict(args: argparse.Namespace) -> None:
    ...
    raw_train_sorted = raw_train_sorted.drop(columns=["__orig_order_beta03"], errors="ignore")
    ...
    raw_valid_sorted_clean = raw_valid_sorted.drop(columns=["__orig_order_beta03"], errors="ignore")
    ...
    for i, cfg in enumerate(candidates):
        ...
        write_submission(submissions_dir / f"beta03_{cfg.name}.csv", pred_out)
        summarize_pred(reports_dir / f"beta03_{cfg.name}.summary.json", pred_out, {"kind": "single_model"})
        for strength in (0.25, 0.45):
            ...
            write_submission(submissions_dir / f"beta03_{cfg.name}_rules{int(strength*100)}.csv", p2_out)
            summarize_pred(reports_dir / f"beta03_{cfg.name}_rules{int(strength*100)}.summary.json", p2_out, {"kind": "single_model_rules", "rules_strength": strength})
    ...
    ensembles["ensemble_legacy_conservative"] = 0.80 * p["legacy_exact_1892"] + 0.20 * p["legacy_depth4_2200"]
    ensembles["ensemble_mean_all"] = np.mean(np.vstack([p[k] for k in p]), axis=0)

    for name, pred in ensembles.items():
        pred = clip_pred(pred, raw_valid_sorted_clean, args.capacity_mw)
        out = restore_order(pred, valid_orig)
        write_submission(submissions_dir / f"beta03_{name}.csv", out)
        summarize_pred(reports_dir / f"beta03_{name}.summary.json", out, {"kind": "ensemble"})
        for strength in (0.15, 0.25, 0.45):
            p2 = clip_pred(pred + rule_correction(fe_valid, pred, rule_state, strength), raw_valid_sorted_clean, args.capacity_mw)
            out2 = restore_order(p2, valid_orig)
            write_submission(submissions_dir / f"beta03_{name}_rules{int(strength*100)}.csv", out2)
            summarize_pred(reports_dir / f"beta03_{name}_rules{int(strength*100)}.summary.json", out2, {"kind": "ensemble_rules", "rules_strength": strength})
    ...
    joblib.dump(artifact, artifacts_dir / "beta03_production_ensemble.pkl")
    (artifacts_dir / "beta03_feature_columns.json").write_text(json.dumps(feature_cols, ensure_ascii=False, indent=2), encoding="utf-8")
    (artifacts_dir / "beta03_rule_state.json").write_text(json.dumps(rule_state, ensure_ascii=False, indent=2), encoding="utf-8")
    if cv_rows:
        (artifacts_dir / "beta03_cv_metrics.json").write_text(json.dumps(cv_rows, ensure_ascii=False, indent=2), encoding="utf-8")

    first = [
        "beta03_legacy_exact_1892.csv",
        "beta03_legacy_exact_1892_rules25.csv",
        "beta03_legacy_exact_2500.csv",
        "beta03_ensemble_legacy_conservative.csv",
        "beta03_ensemble_legacy_conservative_rules25.csv",
        "beta03_ensemble_legacy_weighted.csv",
        "beta03_ensemble_legacy_weighted_rules25.csv",
        "beta03_ensemble_mean_all.csv",
    ]
    (submissions_dir / "SUBMIT_FIRST.txt").write_text("\n".join(str(submissions_dir / x) for x in first) + "\n", encoding="utf-8")

    print(f"Saved artifact: {artifacts_dir / 'beta03_production_ensemble.pkl'}")
...
def main() -> None:
    ...
    ap.add_argument("--artifacts_dir", default="artifacts_beta03")
    ap.add_argument("--submissions_dir", default="submissions_beta03")
    ap.add_argument("--reports_dir", default="reports_beta03")

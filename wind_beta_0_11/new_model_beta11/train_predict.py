...
def profile_delta(valid_raw: pd.DataFrame, base_pred: np.ndarray, profile: str) -> np.ndarray:
    ...
    if profile in ("physics", "physics_strong", "physics_xstrong", "physics_ultra", "physics_plus_dir_tiny", "theory103", "density_boost", "all_mild", "all_strong"):
        ...
        # so beta11 line-searches around this vector.
...
def main() -> None:
    ...
    p.add_argument("--submission_dir", default="submissions_beta11")
    p.add_argument("--report_dir", default="reports_beta11")
    args = p.parse_args()
    set_seed(args.seed)
    ...
    candidate_specs = [
        # beta11 first wave: tight around beta10 production-best
    ...
    for ens_name, rules_strength, bias, profile in candidate_specs:
        ...
        fname = f"beta11_{ens_name}_rules{int(rules_strength*100):02d}_bias{str(bias).replace('.', 'p')}{prof_suffix}.csv"
    ...
    for ens_name in ["anchor_1700_d4_w70", "anchor_1700_d4_w75", "anchor_1700_d4_w80", "anchor_balanced"]:
        ...
        fname = f"beta11_{ens_name}_raw.csv"
    ...
    (art_dir / "beta11_model_meta.json").write_text(json.dumps({"target_col": target_col, "models": metas, "ensembles": list(ensembles)}, ensure_ascii=False, indent=2), encoding="utf-8")

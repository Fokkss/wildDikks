...
def profile_delta(valid_raw: pd.DataFrame, base_pred: np.ndarray, profile: str) -> np.ndarray:
    ...
    if profile in ("physics", "physics_strong", "physics_xstrong", "physics_ultra", "physics_plus_dir_tiny", "theory103", "density_boost", "all_mild", "all_strong"):
        ...
        # so beta10 line-searches around this vector.
...
def main() -> None:
    ...
    p.add_argument("--artifact_dir", default="artifacts_beta10")
    p.add_argument("--submission_dir", default="submissions_beta10")
    p.add_argument("--report_dir", default="reports_beta10")
    args = p.parse_args()
    set_seed(args.seed)
    ...
    # Beta10: narrow production search around beta08 best:
    # anchor_1700_d4_w75 + rules45 + bias0.75 + physics -> 8.4133.
    # First wave is only 8 candidates because submissions are slow.
    candidate_specs = [
        # first wave: tight line-search around beta10 best
        # beta10 best: anchor_1700_d4_w75 + rules45 + bias0.75 + physics_strong -> 8.4108
        ("anchor_1700_d4_w75", 0.45, 0.75, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.45, 0.80, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.45, 0.70, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.40, 0.75, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.50, 0.75, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.45, 0.75, "physics_ultra"),
        ("anchor_1700_d4_w73", 0.45, 0.75, "physics_strong"),
        ("anchor_1700_d4_w77", 0.45, 0.75, "physics_strong"),
        # second wave: only if first wave beats 8.4108
        ("anchor_1700_d4_w72", 0.45, 0.75, "physics_strong"),
        ("anchor_1700_d4_w78", 0.45, 0.75, "physics_strong"),
        ("anchor_1700_d4_w75", 0.45, 0.72, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.45, 0.78, "physics_xstrong"),
        ("anchor_1700_d4_w75", 0.45, 0.75, "theory103"),
        ("anchor_1700_d4_w75", 0.45, 0.75, "density_boost"),
        ("anchor_1700_d4_w75", 0.55, 0.75, "physics_strong"),
        ("anchor_1700_d4_w70", 0.55, 0.75, "physics_strong"),
    ]
    for ens_name, rules_strength, bias, profile in candidate_specs:
        ...
        fname = f"beta10_{ens_name}_rules{int(rules_strength*100):02d}_bias{str(bias).replace('.', 'p')}{prof_suffix}.csv"
    ...
    for ens_name in ["anchor_1700_d4_w70", "anchor_1700_d4_w75", "anchor_1700_d4_w80", "anchor_balanced"]:
        ...
        fname = f"beta10_{ens_name}_raw.csv"
    ...
    (art_dir / "beta10_model_meta.json").write_text(json.dumps({"target_col": target_col, "models": metas, "ensembles": list(ensembles)}, ensure_ascii=False, indent=2), encoding="utf-8")

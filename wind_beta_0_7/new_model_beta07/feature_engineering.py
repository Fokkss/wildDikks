...
def make_features(df: pd.DataFrame, target_col: Optional[str] = None, *, use_lags: bool = True, use_time: bool = True, use_availability: bool = True) -> pd.DataFrame:
    ...
    out["hub_ws_84m_piece_3_12"] = (hub84.clip(3,12)-3).clip(lower=0)
    pc84 = ((hub84.clip(3,12)-3)/9.0)**3
    ...
    out["below_cut_in"] = (hub84 < 3).astype(float)
    out["rated_zone"] = ((hub84 >= 12) & (hub84 < 25)).astype(float)

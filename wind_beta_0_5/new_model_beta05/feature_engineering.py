...
def wind_direction_to_degrees(raw: pd.Series) -> pd.Series:
    """Normalize wind direction to degrees.

    The competition data we observed stores direction as degrees / 1000
    (for example 0.273 means 273 degrees). Some older exports may already
    contain degrees, so this function auto-detects the scale.
    """
    x = pd.to_numeric(raw, errors="coerce")
    q99 = x.dropna().quantile(0.99) if x.notna().any() else np.nan
    if pd.notna(q99) and q99 <= 1.5:
        return (x * 1000.0) % 360.0
    if pd.notna(q99) and q99 <= 7.0:
        # radians fallback
        return np.rad2deg(x) % 360.0
    return x % 360.0
...
def add_wind_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if v80 is not None:
        ...
        # True hub height in the task is 84m. If 120m exists, estimate 84m by
        # a local power-law exponent between 80 and 120m; otherwise use 80m.
        if speed[120] is not None:
            s80 = v80.clip(lower=0.05)
            s120 = speed[120].clip(lower=0.05)
            alpha_80_120_for_84 = pd.Series(
                _safe_divide(np.log(s120 / s80), math.log(120.0 / 80.0)),
                index=out.index,
            ).clip(-0.5, 1.0)
            hub_ws_84 = v80 * (84.0 / 80.0) ** alpha_80_120_for_84
            out["shear_alpha_80_120_for_84"] = alpha_80_120_for_84
        else:
            hub_ws_84 = v80
        out["hub_ws_84m"] = hub_ws_84
        out["hub_ws_84m_sq"] = hub_ws_84 ** 2
        out["hub_ws_84m_cube"] = hub_ws_84 ** 3
        out["hub_ws_84m_piece_3_12"] = (hub_ws_84.clip(3.0, 12.0) - 3.0).clip(lower=0.0)
        out["power_curve_84m"] = (((hub_ws_84.clip(3.0, 12.0) - 3.0) / 9.0) ** 3).clip(0.0, 1.0)
        out["expected_power_84m_proxy"] = out["power_curve_84m"] * out.get("available_capacity_mw", FARM_CAPACITY_MW)
    ...
    for h in (10, 80, 120, 180):
        ...
        if direction[h] is not None:
            dir_deg = wind_direction_to_degrees(direction[h])
            ...
            if h == 80:
                for lo, hi in ((0,45),(45,60),(60,75),(75,90),(90,105),(105,120),(120,135),(135,180),(180,225),(225,270),(270,315),(315,360)):
                    out[f"dir{lo}_{hi}"] = ((dir_deg >= lo) & (dir_deg < hi)).astype("float")
                out["dir45_90"] = ((dir_deg >= 45) & (dir_deg < 90)).astype("float")
                out["dir90_135"] = ((dir_deg >= 90) & (dir_deg < 135)).astype("float")
    ...
    for h1, h2 in ((10, 80), (80, 120), (80, 180), (10, 120)):
        if direction[h1] is not None and direction[h2] is not None:
            out[f"wd_diff_{h2}_{h1}"] = circular_diff_deg(wind_direction_to_degrees(direction[h2]), wind_direction_to_degrees(direction[h1]))
...
def add_air_density_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if temp is not None:
        temp_c = temp if (temp.dropna().median() if temp.notna().any() else 999) < 150 else temp - 273.15
        out["temp_c_inferred"] = temp_c
        out["temp_gt5"] = (temp_c > 5.0).astype("float")
        out["temp_lt_m10"] = (temp_c < -10.0).astype("float")
    if pressure_pa is not None:
        out["pressure_low"] = (pressure_pa < 100500.0).astype("float")
        out["pressure_midlow"] = ((pressure_pa >= 100500.0) & (pressure_pa < 101500.0)).astype("float")
        out["pressure_high"] = (pressure_pa >= 102500.0).astype("float")
...
def add_weather_misc_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if cloud_cols:
        ...
        cloud_norm = out["cloud_max_inferred"].copy()
        if cloud_norm.dropna().median() > 1.5:
            cloud_norm = cloud_norm / 100.0
        out["cloud_hi"] = (cloud_norm > 0.70).astype("float")
    return out

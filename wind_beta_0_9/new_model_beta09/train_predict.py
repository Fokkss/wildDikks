...
def profile_delta(valid_raw: pd.DataFrame, base_pred: np.ndarray, profile: str) -> np.ndarray:
    ...
    if profile in ("physics", "physics_strong", "physics_plus_dir_tiny", "all_mild", "all_strong"):
        # Stability / density / ramp profile. Beta08 best was physics-only,
        # so beta09 line-searches around this vector.
...
def main() -> None:
    ...
    p.add_argument("--artifact_dir", default="artifacts_beta09")
    p.add_argument("--submission_dir", default="submissions_beta09")
    p.add_argument("--report_dir", default="reports_beta09")
    args = p.parse_args()
    set_seed(args.seed)
    ...
    # Beta09: narrow production search around beta08 best:
    # anchor_1700_d4_w75 + rules45 + bias0.75 + physics -> 8.4133.
    # First wave is only 8 candidates because submissions are slow.
    candidate_specs = [
        # first wave: bias/rules/weight line-search around physics-only profile
        ("anchor_1700_d4_w75", 0.45, 0.65, "physics"),
        ("anchor_1700_d4_w75", 0.45, 0.70, "physics"),
        ("anchor_1700_d4_w75", 0.45, 0.80, "physics"),
        ("anchor_1700_d4_w75", 0.55, 0.70, "physics"),
        ("anchor_1700_d4_w75", 0.55, 0.75, "physics"),
        ("anchor_1700_d4_w70", 0.45, 0.70, "physics"),
        ("anchor_1700_d4_w70", 0.45, 0.75, "physics"),
        ("anchor_1700_d4_w65", 0.45, 0.75, "physics"),
        # second wave: only if first wave beats 8.4133
        ("anchor_1700_d4_w75", 0.45, 0.70, "physics_strong"),
        ("anchor_1700_d4_w75", 0.45, 0.75, "physics_strong"),
        ("anchor_1700_d4_w75", 0.50, 0.70, "physics"),
        ("anchor_1700_d4_w75", 0.50, 0.75, "physics"),
        ("anchor_1700_d4_w70", 0.55, 0.70, "physics"),
        ("anchor_1700_d4_w70", 0.55, 0.75, "physics"),
        ("anchor_1700_d4_w75", 0.45, 0.70, "physics_plus_dir_tiny"),
        ("anchor_1700_d4_w75", 0.55, 0.70, "physics_plus_dir_tiny"),
    ]
    for ens_name, rules_strength, bias, profile in candidate_specs:
        ...
        fname = f"beta09_{ens_name}_rules{int(rules_strength*100):02d}_bias{str(bias).replace('.', 'p')}{prof_suffix}.csv"
    ...
    for ens_name in ["anchor_1700_d4_w70", "anchor_1700_d4_w75", "anchor_1700_d4_w80", "anchor_balanced"]:
        ...
        fname = f"beta09_{ens_name}_raw.csv"
    ...
    (art_dir / "beta09_model_meta.json").write_text(json.dumps({"target_col": target_col, "models": metas, "ensembles": list(ensembles)}, ensure_ascii=False, indent=2), encoding="utf-8")

...
def sort_for_features_keep_order(df: pd.DataFrame) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    tmp = df.copy()
    tmp["__orig_order_beta06"] = np.arange(len(tmp))
    if dt_col is not None:
        tmp["__dt_sort_beta06"] = pd.to_datetime(tmp[dt_col], errors="coerce")
        tmp = tmp.sort_values("__dt_sort_beta06", kind="mergesort").drop(columns=["__dt_sort_beta06"])
    return tmp.reset_index(drop=True), tmp["__orig_order_beta06"].to_numpy(int)
...
def make_features_chrono(df: pd.DataFrame, target_col: str | None = None) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    fe = make_features(sorted_df.drop(columns=["__orig_order_beta06"], errors="ignore"), target_col=target_col)
...
def candidate_configs() -> list[CandidateConfig]:
    # beta 0.6: focus on the reproducible legacy family.
    # We keep a small set to reduce runtime and submissions.
    return [
        CandidateConfig("legacy_exact_1700", 1700, 0.03, 3, 5, 0.85, 0.85, 0.04, 5.0),
        CandidateConfig("legacy_exact_1892", 1892, 0.03, 3, 5, 0.85, 0.85, 0.04, 5.0),
        CandidateConfig("legacy_exact_2100", 2100, 0.03, 3, 5, 0.85, 0.85, 0.04, 5.0),
...
def build_xgb(cfg: CandidateConfig, seed: int, n_estimators_override: int | None = None, early_stopping_rounds: int | None = None):
    try:
        ...
        raise RuntimeError("xgboost is required for beta 0.6 production training") from e
    model = XGBRegressor(
...
def extra_profile_correction(fe: pd.DataFrame, pred: np.ndarray, profile: str) -> np.ndarray:
    """Small production-safe profile corrections discovered by diagnostics.

    These do not use any external CSV/teacher; they are deterministic feature-based
    corrections. Keep magnitudes small because this branch targets production.
    """
    pred_s = pd.Series(np.asarray(pred, dtype=float), index=fe.index)
    corr = pd.Series(0.0, index=fe.index, dtype=float)
    ws = col(fe, "hub_ws_84m", np.nan)
    ...
    if profile == "dircloud_more":
        corr += -0.16 * col(fe, "dir60_75")
        corr += -0.08 * col(fe, "dir45_90")
        corr +=  0.06 * col(fe, "dir315_360")
        corr += -0.10 * col(fe, "cloud_hi")
    elif profile == "physics_more":
        corr +=  0.14 * col(fe, "pressure_low")
        corr += -0.11 * col(fe, "pressure_midlow")
        corr +=  0.10 * col(fe, "temp_gt5")
        corr +=  0.08 * ((ws >= 5.0) & (ws < 7.0)).astype(float)
        corr +=  0.08 * ((ws >= 10.0) & (ws < 11.0)).astype(float)
        corr += -0.05 * (ws >= 13.0).astype(float)
    elif profile == "dircloud_physics":
        corr += extra_profile_correction(fe, pred, "dircloud_more")
        corr += extra_profile_correction(fe, pred, "physics_more")
    elif profile == "lowwind_relax":
        corr += 0.12 * ((ws < 3.0) & (pred_s < 15.0)).astype(float)
        corr += 0.05 * (pred_s < 15.0).astype(float)
    else:
...
def train_and_predict(args: argparse.Namespace) -> None:
    ...
    raw_train_sorted = raw_train_sorted.drop(columns=["__orig_order_beta06"], errors="ignore")
    ...
    raw_valid_sorted_clean = raw_valid_sorted.drop(columns=["__orig_order_beta06"], errors="ignore")
    ...
    def fit_predict_model(name: str, cfg: CandidateConfig, seed: int, n_estimators_override: int | None = None) -> np.ndarray:
        ...
        write_submission(submissions_dir / f"beta06_{name}.csv", pred_out)
        summarize_pred(reports_dir / f"beta06_{name}.summary.json", pred_out, {"kind": "single_model", "n_estimators": int(model.get_params().get("n_estimators", 0))})
        return pred
    ...
    for i, cfg in enumerate(candidates):
        ...
        for strength in (0.25, 0.55, 0.45):
            ...
            write_submission(submissions_dir / f"beta06_{cfg.name}_rules{int(strength*100)}.csv", p2_out)
            summarize_pred(reports_dir / f"beta06_{cfg.name}_rules{int(strength*100)}.summary.json", p2_out, {"kind": "single_model_rules", "rules_strength": strength})
    ...
    if args.run_es:
        ...
        try:
            ...
            n_est = int(np.clip(round(depth4_best), 300, 10000))
            name = f"depth4_es{n_est}"
            fit_predict_model(name, depth4_es_cfg, args.seed + 1020, n_estimators_override=n_est)
        except Exception as e:
    ...
    ensembles["legacy_conservative_1892_d4_w80"] = 0.80 * p["legacy_exact_1892"] + 0.20 * p["legacy_depth4_2200"]
    ensembles["legacy_conservative_1700_d4_w80"] = 0.80 * p["legacy_exact_1700"] + 0.20 * p["legacy_depth4_2200"]
    ensembles["legacy_conservative_1700_d4_w75"] = 0.75 * p["legacy_exact_1700"] + 0.25 * p["legacy_depth4_2200"]
    ensembles["legacy_conservative_1700_d4_w70"] = 0.70 * p["legacy_exact_1700"] + 0.30 * p["legacy_depth4_2200"]
    ensembles["legacy_weighted"] = 0.55 * p["legacy_exact_1700"] + 0.25 * p["legacy_exact_1892"] + 0.20 * p["legacy_depth4_2200"]
    ensembles["legacy_mean_core"] = np.mean(np.vstack([p[k] for k in ["legacy_exact_1700", "legacy_exact_1892", "legacy_depth4_2200"]]), axis=0)
    ...
    def save_variant(name: str, pred_sorted: np.ndarray, kind: str, rules_strength: float = 0.0, bias: float = 0.0) -> None:
        ...
        write_submission(submissions_dir / f"beta06_{name}.csv", out)
        summarize_pred(
            reports_dir / f"beta06_{name}.summary.json",
            out,
            {"kind": kind, "rules_strength": rules_strength, "bias": bias},
        )
        saved_submission_names.append(f"beta06_{name}.csv")

    # Focused variants: production-only grid around the best beta05 signal.
    for name, pred in ensembles.items():
        pred = clip_pred(pred, raw_valid_sorted_clean, args.capacity_mw)
        save_variant(name, pred, "ensemble", 0.0)
        for strength in (0.35, 0.45, 0.55, 0.60, 0.75):
            p2 = clip_pred(pred + rule_correction(fe_valid, pred, rule_state, strength), raw_valid_sorted_clean, args.capacity_mw)
            save_variant(f"{name}_rules{int(strength*100)}", p2, "ensemble_rules", strength)

    # Bias and profile candidates around the strongest reproducible families.
    focus_names = [
        "legacy_conservative_1892_d4_w80",
        "legacy_conservative_1700_d4_w80",
        "legacy_conservative_1700_d4_w75",
        "legacy_weighted",
    ]
    for name in focus_names:
        ...
        for strength in (0.35, 0.45, 0.60):
            ...
            for bias in (0.40, 0.55, 0.70, 0.85, 1.00, 1.20):
                save_variant(f"{name}_rules{int(strength*100)}_bias{str(bias).replace('.', 'p')}", p_rules, "ensemble_rules_bias", strength, bias)

            # deterministic feature profiles, still self-contained.
            for profile in ("dircloud_more", "physics_more", "dircloud_physics", "lowwind_relax"):
                p_prof = clip_pred(p_rules + extra_profile_correction(fe_valid, p_rules, profile), raw_valid_sorted_clean, args.capacity_mw)
                save_variant(f"{name}_rules{int(strength*100)}_{profile}", p_prof, f"ensemble_rules_profile:{profile}", strength, 0.0)
                for bias in (0.40, 0.70):
                    save_variant(f"{name}_rules{int(strength*100)}_{profile}_bias{str(bias).replace('.', 'p')}", p_prof, f"ensemble_rules_profile_bias:{profile}", strength, bias)
    ...
    artifact = {
        ...
        "default_ensemble": "legacy_conservative_1892_d4_w80",
        "seed": args.seed,
    }
    joblib.dump(artifact, artifacts_dir / "beta06_production_ensemble.pkl")
    (artifacts_dir / "beta06_feature_columns.json").write_text(json.dumps(feature_cols, ensure_ascii=False, indent=2), encoding="utf-8")
    (artifacts_dir / "beta06_rule_state.json").write_text(json.dumps(rule_state, ensure_ascii=False, indent=2), encoding="utf-8")
    (artifacts_dir / "beta06_es_report.json").write_text(json.dumps(es_report, ensure_ascii=False, indent=2), encoding="utf-8")
    if cv_rows:
        (artifacts_dir / "beta06_cv_metrics.json").write_text(json.dumps(cv_rows, ensure_ascii=False, indent=2), encoding="utf-8")

    first = [
        # Submit these 8 first: bias search around current best and two plausible variants.
        "beta06_legacy_conservative_1892_d4_w80_rules45_bias0p55.csv",
        "beta06_legacy_conservative_1892_d4_w80_rules45_bias0p70.csv",
        "beta06_legacy_conservative_1892_d4_w80_rules45_bias0p85.csv",
        "beta06_legacy_conservative_1892_d4_w80_rules60_bias0p55.csv",
        "beta06_legacy_conservative_1892_d4_w80_rules60_bias0p70.csv",
        "beta06_legacy_conservative_1700_d4_w80_rules45_bias0p55.csv",
        "beta06_legacy_conservative_1700_d4_w75_rules45_bias0p55.csv",
        "beta06_legacy_weighted_rules45_bias0p55.csv",
        # If one of the first 8 improves, try these profile candidates next.
        "beta06_legacy_conservative_1892_d4_w80_rules45_dircloud_more_bias0p40.csv",
        "beta06_legacy_conservative_1892_d4_w80_rules45_physics_more_bias0p40.csv",
        "beta06_legacy_conservative_1892_d4_w80_rules45_dircloud_physics_bias0p40.csv",
        "beta06_legacy_conservative_1700_d4_w80_rules45_dircloud_more_bias0p40.csv",
    ]
    (submissions_dir / "SUBMIT_FIRST.txt").write_text("\n".join(str(submissions_dir / x) for x in first) + "\n", encoding="utf-8")

    print(f"Saved artifact: {artifacts_dir / 'beta06_production_ensemble.pkl'}")
...
def main() -> None:
    ...
    ap.add_argument("--artifacts_dir", default="artifacts_beta06")
    ap.add_argument("--submissions_dir", default="submissions_beta06")
    ap.add_argument("--reports_dir", default="reports_beta06")
    ...
    ap.add_argument("--run_es", action="store_true", help="Optional slow early-stopping probes; disabled by default because beta05 ES underpredicted.")
    args = ap.parse_args()

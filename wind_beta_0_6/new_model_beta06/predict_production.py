...
def make_ensemble(preds: dict[str, np.ndarray], name: str) -> np.ndarray:
    if name == "legacy_conservative_1892_d4_w80":
        return 0.80 * preds["legacy_exact_1892"] + 0.20 * preds["legacy_depth4_2200"]
    if name == "legacy_conservative_1700_d4_w80":
        return 0.80 * preds["legacy_exact_1700"] + 0.20 * preds["legacy_depth4_2200"]
    if name == "legacy_conservative_1700_d4_w75":
        return 0.75 * preds["legacy_exact_1700"] + 0.25 * preds["legacy_depth4_2200"]
    if name == "legacy_conservative_1700_d4_w70":
        return 0.70 * preds["legacy_exact_1700"] + 0.30 * preds["legacy_depth4_2200"]
    if name == "legacy_weighted":
        return 0.55 * preds["legacy_exact_1700"] + 0.25 * preds["legacy_exact_1892"] + 0.20 * preds["legacy_depth4_2200"]
    if name == "legacy_mean_core":
        return np.mean(np.vstack([preds[k] for k in ["legacy_exact_1700", "legacy_exact_1892", "legacy_depth4_2200"]]), axis=0)
    if name in preds:
        return preds[name]
...
def main():
    ...
    ap.add_argument("--model_path", default="artifacts_beta06/beta06_production_ensemble.pkl")
    ap.add_argument("--output_path", default="submissions_beta06/submission_2027_beta06_production.csv")
    ap.add_argument("--ensemble", default="legacy_conservative_1892_d4_w80")
    ap.add_argument("--rules_strength", type=float, default=0.45)
    ap.add_argument("--bias", type=float, default=0.55)
    ...
    raw_sorted_clean = raw_sorted.drop(columns=["__orig_order_beta06"], errors="ignore")
